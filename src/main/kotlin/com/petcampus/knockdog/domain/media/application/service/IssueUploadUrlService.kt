...
import com.petcampus.knockdog.domain.media.domain.MediaPurpose
...
class IssueUploadUrlService(
    ...
    override fun issue(command: IssueUploadUrlCommand): UploadUrl {
        val purpose =
            MediaPurpose.from(command.purpose)
                ?: throw BusinessException(MediaErrorCode.UNSUPPORTED_PURPOSE, command.purpose)

        val contentType =
            MediaContentType.forMimeType(command.contentType)
                ?: throw BusinessException(MediaErrorCode.UNSUPPORTED_CONTENT_TYPE, command.contentType)

        val key = ObjectKey.temporary(command.userCode, purpose, contentType)
