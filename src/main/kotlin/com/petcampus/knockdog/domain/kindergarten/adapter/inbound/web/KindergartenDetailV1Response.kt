...
/**
 * `GET /api/v1/kindergartens/{id}/detail` 응답 — `v0`(`basic/{id}`, [KindergartenDetailResponse])의 재설계판.
 *
 * 레거시 버그 이식이었던 `breakTime` 필드를 뺐다 — 크롤링 데이터에 브레이크타임 개념 자체가 없어서 `v0`도
 * 실제로는 마감 시각을 잘못 채우고 있었다. `address`/`roadAddress`도 [KindergartenSummaryV1Response]와
 * 동일하게 분리했다.
 */
data class KindergartenDetailV1Response(
    val id: String,
    val address: String,
    ...
    companion object {
        fun from(kindergarten: Kindergarten): KindergartenDetailV1Response =
            KindergartenDetailV1Response(
                ...
                businessHours = kindergarten.businessHours.mapNotNull { it.toV1ResponseOrNull() },
        ...
        private fun KindergartenBusinessHour.toV1ResponseOrNull(): BusinessHours? {
