...
class MemoServiceTest {
    private class FakeMemoPort :
        ...
        override fun save(memo: Memo): Memo {
            val persisted =
                if (memo.id == null) {
                    Memo.reconstitute(MemoId(seq++), memo.userCode, memo.targetId, memo.content, memo.photos)
                } else {
                    memo
                }
    ...
    fun `save는 없으면 생성, 있으면 content 교체`() {
        ...
        svc.save(SaveMemoCommand("A1B2C3D4", "place-1", "처음", emptyList()))
        svc.save(SaveMemoCommand("A1B2C3D4", "place-1", "수정됨", emptyList()))
    ...
    fun `content가 2000자를 넘으면 MEMO_CONTENT_TOO_LONG`() {
        val exception =
            assertFailsWith<BusinessException> {
                service().save(SaveMemoCommand("A1B2C3D4", "place-1", "가".repeat(2001), emptyList()))
            }
    ...
    fun `list는 유치원당 1건을 shopId·memoDate와 함께 반환`() {
        val svc = service()
        svc.save(SaveMemoCommand("A1B2C3D4", "place-1", "a", emptyList()))
    ...
    fun `save는 tmp key를 commit해 영구 key로 저장하고 GET에서 url을 채운다`() {
        val svc = service()
        svc.save(SaveMemoCommand("A1B2C3D4", "place-1", "m", listOf("tmp/A1B2C3D4/MEMO_ATTACHMENT/u.webp")))
    ...
    fun `이미 영구인 내 memo 소유 key는 그대로 유지된다`() {
        val svc = service()
        svc.save(SaveMemoCommand("A1B2C3D4", "p", null, listOf("memo/A1B2C3D4/kept.webp")))
    ...
    fun `남의 네임스페이스 key는 MEMO_INVALID_PHOTO_KEY`() {
        val exception =
            assertFailsWith<BusinessException> {
                service().save(SaveMemoCommand("A1B2C3D4", "p", null, listOf("memo/OTHER999/x.webp")))
            }
    ...
    fun `사진 6장이면 MEMO_TOO_MANY_PHOTOS`() {
        ...
        val exception =
            assertFailsWith<BusinessException> {
                service().save(SaveMemoCommand("A1B2C3D4", "p", null, keys))
            }
