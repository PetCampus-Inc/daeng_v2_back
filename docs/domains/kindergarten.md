...
- **프론트에 전달 필요 — SNS 링크(홈페이지/인스타그램/유튜브)를 한 유치원이 여러 개 가질 수 있다**: 실제 크롤링 데이터 435건 중 19건이 같은 종류(예: INSTAGRAM)의 링크를 2개씩 갖고 있다(서로 다른 URL — 계정을 두 개 운영). `kindergarten_links` 테이블·도메인 모델은 이미 유치원당 링크를 여러 개 담을 수 있게 돼 있지만(`V7`에서 `UNIQUE(kindergarten_id, code)`를 `(kindergarten_id, code, url)`로 완화), `daeng_v2_front`가 `homepageUrl`/`instagramUrl`/`youtubeUrl`을 화면·원장 편집 폼·타입 정의 어디서나 단일 문자열로만 다뤄서, API 응답(`linkOf()`)도 지금은 code당 첫 값만 반환한다. 프론트가 여러 개를 보여줄 UI를 만들면 그때 응답을 배열로 바꾼다 — 프론트 확인·논의 필요(사람 몫).
