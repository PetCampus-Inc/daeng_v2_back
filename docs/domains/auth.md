> 생성: 2026-07-27 17:51 · 최종 수정: 2026-07-28 17:00

# auth 도메인 마이그레이션 지시서

- 설계 근거: [`0001`](../adr/0001-legacy-v1-v2-폐기.md) 레거시 폐기, [`0002`](../adr/0002-db-스키마-유지.md)(→[`0010`](../adr/0010-신규-db-인스턴스-스키마-재작성.md)으로 대체됨) 스키마, [`0003`](../adr/0003-헥사고날-정석형-통일.md) 헥사고날 정석형, [`0004`](../adr/0004-api-v0-유지-v1-신규.md) API v0/v1 + 작업 단위 분해, [`0006`](../adr/0006-소셜로그인-id-token-직접검증-유지.md) 소셜 로그인 ID Token 유지. 전체 목록은 [ADR 인덱스](../adr/README.md) 참고
- 원본: `knockdog_server` 의 `auth/` 패키지 (컨트롤러 2개, 서비스 5개, 총 58파일)
- 슬라이스 규칙: [`docs/architecture/hexagonal.md`](../architecture/hexagonal.md) (정석형 — `domain/` 순수 모델 + JPA 엔티티 분리 + 매퍼, ArchUnit 경계 강제). 실제 코드 예시는 `domain/owner/`
- **스키마 재검토 필요** ([`0010`](../adr/0010-신규-db-인스턴스-스키마-재작성.md)) — 신규 DB 인스턴스에 스키마를 새로 작성하기로 바뀌어서, 아래 §0 테이블은 기존(레거시) 스키마 기준 참고 자료다. 신규 스키마 확정 전까지는 필드명/제약을 그대로 신뢰하지 말 것
...
| 저장소 | 이름 | 비고 |
|---|---|---|
| MySQL (JPA) | `user` | PK는 auto-increment `Long`, 대외 식별자는 `user_id`(8자 영숫자, `ShortIdGenerator`) |
| MySQL (JPA) | `user_address` | `user`에 `@ManyToOne`. HOME 주소가 최소 1개 있어야 함(도메인 불변식) |
| MySQL (JPA) | `social_user` | `(provider, provider_id)` 유니크. `user_id`는 nullable(연동 전 상태 존재) |
| MySQL (JPA) | `withdraw_reason` | `user`에 `@ManyToOne(nullable=true)` — 탈퇴 유저 배치 삭제 후에도 사유 레코드는 남을 수 있음 |
| Redis (`@RedisHash`) | `refresh_token` | TTL 30일. **kindergarten의 `flushDb()`와 같은 DB를 공유하므로 D5(무조건 flushDb 제거) 반드시 함께 반영** |
| Redis (`@RedisHash`) | `email_verification` | TTL은 시도제한(30분)+만료(3분) 합산, 인증 완료 시 10분으로 재설정 |

## 1. 마이그레이션 대상 엔드포인트 (14 → 12)

| 기존 (`v0`, 그대로 유지) | 신규 (`v1`) | 판정 |
|---|---|---|
| `POST /api/v0/auth/verify/oidc` | `POST /api/v1/auth/oidc-verifications` | 이관 |
| `POST /api/v0/auth/login` | `POST /api/v1/auth/sessions` | 이관 |
| `POST /api/v0/auth/logout` | `DELETE /api/v1/auth/sessions` | 이관 |
| `POST /api/v0/auth/refresh` | `PATCH /api/v1/auth/sessions` | 이관 |
| `POST /api/v0/auth/email/send` | `POST /api/v1/email-verifications` | 이관 |
| `POST /api/v0/auth/email/verify` | `PATCH /api/v1/email-verifications` | 이관 |
| `GET /api/v0/auth/email/verification` | `GET /api/v1/email-verifications` | 이관 |
| `GET /api/v0/auth/dev/{id}` | — (`v1` 미제공) | 이관, 단 **`@Profile("local")` 가드 신규 추가 필수** (§3, 현재 운영 노출) |
| `GET /api/v0/user/social/user` | `GET /api/v1/users/me/social-account` | 이관 |
| `POST /api/v0/user/social/reconnect` | `PUT /api/v1/users/me/social-account` | 이관 |
| `POST /api/v0/user/register` | `POST /api/v1/users` | 이관 |
| `POST /api/v0/user/withdraw` | `DELETE /api/v1/users/me` | 이관 |
