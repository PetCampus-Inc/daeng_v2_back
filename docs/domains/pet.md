> 생성: 2026-09-02 22:02 · 최종 수정: 2026-09-04 19:48
...
| `weight` | 컬럼 타입은 DOUBLE(반려동물 체중은 소수점 단위가 실제로 의미 있어 확장성을 열어둠). 현재 기획(1~99 정수)에 맞춰 값이 있으면 범위와 "소수점 없음"을 검증한다. **생성(`Pet.create`) 시점엔 필수**(non-null `Double`, 레거시 등록 API와 동일) — 다만 DB 컬럼 자체는 nullable이라 등록 후 PATCH로 지울 수 있다 |
