...
| 사용자 식별자 | pet 도메인·application·port는 `UserId`를 사용한다. `pets.user_id`와 `representative_user_id`의 `Long` 값 변환은 persistence adapter에서만 수행한다. 인증 요청 경계의 `UserCode`는 service가 사용자 조회에만 사용한다. |
