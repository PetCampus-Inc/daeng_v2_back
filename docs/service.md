> 생성: 2026-07-28 16:00 · 최종 수정: 2026-07-28 16:54
...
무엇을 버리고, 무엇을 그대로 옮기고, 무엇을 다시 설계할지에 대한 근거와 결정, 엔드포인트 판정, 작업 단위 분해는 [ADR 인덱스](adr/README.md)(`0001`~`0010`)에 있다. 이 문서는 "왜 지금 코틀린으로 마이그레이션하는가"의 요약만 담고, 세부 판단 근거는 그쪽이 단일 출처(single source of truth)다.
...
- Java → Kotlin 마이그레이션 설계 상세: [ADR 인덱스](adr/README.md)
