...
- **`GlobalExceptionHandler`에 `IllegalStateException` → 409(`CommonErrorCode.CONFLICT`) 전역 핸들러를 추가했다가 이후 철회했다.** 처음엔 `Pet.delete()`/`Pet.markAsRepresentative()`(KD3-433/434에서 쓰일 예정)의 `check()` 검증 실패가 지금까지 전용 핸들러 없이 catch-all(500)로 떨어지던 구멍을 독립 리뷰에서 발견해 pet 범위를 넘는 공통 안전망으로 추가했다. 추가 전 grep으로 `check()` 호출부를 전수 확인했는데, 처음엔 3곳(`Pet.delete`/`Pet.markAsRepresentative`/`User.withdraw`)만 확인했고 `PetPersistenceAdapter.registerWithinLimit`의 `check()`를 놓쳐 실제로는 4곳이었다(2차 독립 리뷰에서 정정) — 이 4번째는 `CreatePetService`가 이미 명시적으로 `catch (e: IllegalStateException)`으로 잡아 `BusinessException(LIMIT_EXCEEDED)`로 변환하므로 이 핸들러와는 무관했다.
  - **철회 이유**: PR 리뷰(CodeRabbit)가 "이 핸들러는 `IllegalStateException` 타입 자체를 전역으로 잡기 때문에, pet과 무관한 다른 도메인(auth 등)에서 발생하는 예상 못한 버그까지 409로 오분류하고 그 예외 메시지까지 노출할 위험이 있다"고 지적했다. 사용자가 이 지적에 동의하되, 전용 예외 타입(예: `ConflictException`)을 새로 만들어 범위를 좁히는 대신 **전역 처리 자체를 철회**하기로 결정했다 — `Pet.delete()`/`Pet.markAsRepresentative()`를 실제로 구현할 KD3-433/434에서, `CreatePetService`가 `registerWithinLimit`을 처리하는 것과 동일한 패턴(서비스 레이어에서 명시적으로 `catch`해 그 API에 맞는 `BusinessException`으로 변환)으로 각자 처리하기로 했다. `GlobalExceptionHandler.kt`의 핸들러, `CommonErrorCode.CONFLICT`, `GlobalExceptionHandlerTest`의 대응 테스트를 모두 제거하고 `docs/conventions/error-handling.md`도 함께 되돌렸다.
...
| `GlobalExceptionHandler.kt`/`CommonErrorCode.kt`/`GlobalExceptionHandlerTest.kt` | 추가 후 철회 | `IllegalStateException` → 409 전역 핸들러를 추가했다가, PR 리뷰 지적(pet 무관 도메인의 예상 못한 버그까지 409로 오분류될 위험)에 따라 사용자가 전역 처리 자체를 철회하기로 결정 — 관련 코드·테스트 전부 제거 |
| `docs/conventions/error-handling.md` | 갱신 후 되돌림 | `IllegalStateException` 처리 우선순위·`check()` 사용 기준을 추가했다가 위 철회에 맞춰 원상 복귀, `check()` 상태 위반은 각 티켓이 서비스 레이어에서 명시적으로 처리한다는 원칙만 한 줄로 남김 |
