> 생성: 2026-07-28 23:34 · 최종 수정: 2026-08-02 14:55
...
global/exception/
  ...
  GlobalExceptionHandler.kt  수정 — BusinessException 핸들러 추가, 응답 바디를 Response<T>로 교체
                              ...
                              MissingServletRequestParameterException(400) 전용 핸들러 추가
...
PR 리뷰 코멘트로 `MissingServletRequestParameterException`(필수 `@RequestParam` 누락)도 같은 패턴으로 요청받아 함께 추가했다(400 + `CommonErrorCode.INVALID_INPUT_VALUE`). 이 예외는 메시지에 내부 구현 정보가 없어(`Required request parameter 'x' ... is not present`) `e.message`를 그대로 노출해도 안전하다고 판단했다 — `HttpMessageNotReadableException`처럼 고정 문구로 대체하지 않았다.
...
- 신규 단위 테스트:
  ...
  - `MissingServletRequestParameterException`(필수 `@RequestParam` 누락)이 500이 아니라 400 + `CommonErrorCode.INVALID_INPUT_VALUE`로 응답하는지
...
- (리뷰·수동 확인에서 발견, 후속 티켓으로 분리 권장) `GlobalExceptionHandler`의 catch-all `Exception` 핸들러가 `HttpRequestMethodNotSupportedException`(405)·`HttpMessageNotReadableException`(400)·`MissingServletRequestParameterException`(400) 외의 프레임워크 예외(예: 존재하지 않는 라우트)는 여전히 전부 500으로 마스킹할 수 있다. `@Valid` 기반 필드별 검증 실패 응답 포맷(티켓 7번 항목) 착수 시 함께 정리한다.
