> 생성: 2026-09-02 19:24 · 최종 수정: 2026-09-08 18:30
...
- 전체 빌드(`./gradlew build`) 163건 전부 통과(기존 152건 + 신규 11건: `DeletePetServiceTest` 5건 + `PetPersistenceAdapterTest` 5건 + `PetDeleteAndPromoteConcurrencyTest` 1건 — 락 재조회 실패 시 404로 고친 것과 그 재발 방지 테스트가 이미 이 11건에 포함됨), 실패·에러 0건. **이후 epic으로 rebase**하면서 KD3-432가 추가한 테스트 2건(breed 참조 누락 회귀 테스트)이 베이스에 합류해 최종 174건으로 늘어났다 — 이 최종 상태는 로컬 실행뿐 아니라 커밋 `4cf3c23` 기준 GitHub Actions `build` 체크 통과로도 재확인했다: https://github.com/PetCampus-Inc/daeng_v2_back/actions/runs/34207092945/job/101999016110
...
- **`DeletePetServiceTest`의 실패 경로 테스트가 예외 타입만 확인하고 `errorCode`는 안 봄(수정 완료)**: `assertFailsWith<BusinessException>`만 있고 그 결과를 변수로 안 받아서, 예를 들어 "NOT_AUTHORIZED를 던진다"는 테스트가 실제로는 `NOT_FOUND`가 나와도 통과하는 상태였다. `PetErrorCode`·`assertEquals` import를 추가해 3건(존재하지 않는 pet, 이미 삭제된 pet, 타인 pet) 전부 `exception.errorCode`를 명시적으로 단언하도록 고쳤다. 같은 패턴(예외 타입만 확인)이 `CreatePetServiceTest`/`UpdatePetServiceTest`/`SetRepresentativeServiceTest`/`GetPetServiceTest`에도 있으나 전부 이미 epic에 머지된 파일이라 이번 수정 범위 밖으로 뒀다(사용자 확인 필요, 별도 후속).
...
| `PetPersistenceAdapterTest.kt` | 갱신 | `deleteAndPromoteWithinLock` 테스트 5건 추가(이름순 승격·대표견 없음·UNIQUE 안 걸림·비대표견 무영향·잠금 재조회 실패 시 NOT_FOUND) |
