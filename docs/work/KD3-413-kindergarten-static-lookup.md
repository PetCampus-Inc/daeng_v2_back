...
- **컬럼명(필드명)에 약어 금지**: 리뷰 중 `KindergartenMenu.unitStr`/`totalDurationStr`의 `Str`이 무슨 뜻인지 알아보기 어렵다는 지적으로, `unitLabel`/`totalDurationLabel`로 개명(도메인/JPA 컬럼/응답 DTO 전부). 크롤링 원본 JSON의 실제 필드명(`unit_str`/`total_duration_str`)은 외부 계약이라 그대로 두고, `CrawledMenu`에서만 `@JsonProperty`로 명시 매핑해 내부 이름과 분리했다. DB 컬럼명은 `V5__kindergarten_menus_rename_label_columns.sql`로 리네임(V3/V4는 이미 커밋된 마이그레이션이라 직접 수정하지 않고 새로 추가 — `database-change.md`). 이어서 전체 도메인을 훑어 `CrawledKindergarten.tel`/`lat`/`lng`도 같은 문제라 `phoneNumber`/`latitude`/`longitude`로 개명했다(시딩 전용 DTO라 API 계약과 무관). 컨벤션 자체는 [`docs/conventions/code-style.md`](../conventions/code-style.md)(신규)와 `AGENTS.md`에 일반 규칙으로 기록했다.
- **API 응답/쿼리파라미터의 `lat`/`lng`는 그대로 둔다**: 위 약어 금지 원칙과 별개로, `KindergartenSummaryResponse`/`KindergartenDetailResponse`의 응답 필드와 컨트롤러의 `@RequestParam`은 `latitude`/`longitude`로 바꾸지 않기로 결정했다. auth 도메인이 이미 `lat`/`lng`로 출시했고(`UserAddress`, `AuthUserResponse` 등), `lat`/`lng`는 `id`/`url`처럼 업계에서 이미 표준 축약으로 굳어진 표현이라 `Str` 같은 모호함과는 성격이 다르다고 판단했다(2026-09-02, 사용자 승인).
...
| `docs/conventions/code-style.md` | 코드를 쓸 때마다 참조하는 판단 기준(주석 금지, 식별자 약어 금지)이 새로 정해짐 | **신규 작성** — `docs/README.md`(폴더 구조 표), `AGENTS.md`(최소 원칙 링크)도 함께 갱신 |
