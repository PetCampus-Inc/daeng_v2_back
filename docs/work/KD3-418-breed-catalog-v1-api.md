> 생성: 2026-09-02 19:24 · 최종 수정: 2026-09-02 23:15
...
- 현재 공통 단계: `5`
- 다음 결정 또는 전환 조건: 독립 리뷰에서 작업 문서와 실제 diff를 대조한 뒤 PR 준비로 전환한다.
...
- `query`는 공백을 제거해 한글명 또는 개별 별칭에 검색어가 포함된 전체 견종을 검색한다. 검색 결과는 검색어로 시작하는 항목을 먼저, 시작하지 않지만 포함하는 항목을 다음으로 정렬하고, 각 동순위에서는 한글명 가나다순으로 정렬한다. 검색 결과가 없으면 빈 목록을 반환한다.
