...
시작 선택지 순서에 맞춰 번호를 부여한 workflow를 관리한다. `000-common.md`는 모든 Jira 티켓의 유일한 공통 순서와 `work/` 문서 기본 양식이고, `001`·`002`는 읽기 전용 진입 절차다. `003`부터 `008`까지는 공통 순서를 바꾸지 않고 각 단계의 추가 조건만 더하는 작업 유형별 절차다. `009` 기타는 사용자의 자유 서술을 분류하는 라우팅 항목이므로 별도 파일을 두지 않는다.
