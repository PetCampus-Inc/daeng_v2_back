> 생성: 2026-08-31 01:05 · 최종 수정: 2026-09-07 19:55
...
7. `OptimisticLockingFailureException`(`@Version` 낙관적 락 충돌 — 저장 시점에 다른 트랜잭션이 먼저 같은 행을 바꿔서 버전이 안 맞는 경우) → 409 + `CommonErrorCode.RESOURCE_CONFLICT`(KD3-431, `PetJpaEntity.version` 도입과 함께 추가)
8. `HttpRequestMethodNotSupportedException`(Spring이 던지는 405) → 405 + `CommonErrorCode.METHOD_NOT_ALLOWED`
9. 그 외 `Exception` → 500 + `CommonErrorCode.INTERNAL_SERVER_ERROR`
