> 생성: 2026-07-28 15:04 · 최종 수정: 2026-08-30 14:26

# API 명세서 (Notion) 연동 규칙

API 명세서는 코드 저장소가 아니라 Notion 데이터베이스에서 관리한다. 이 문서는 어떤 AI 도구를 쓰든 동일한 방식으로 그 데이터베이스에 페이지를 생성/수정하는 방법을 정의한다. [`common.md`](../workflows/common.md) 2번(작업 후 확인 목록)·12번(문서 동기화)에서 "API 추가/변경" 작업일 때 이 규칙을 따른다.
