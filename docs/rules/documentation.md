> 생성: 2026-07-28 15:00 · 최종 수정: 2026-09-01 00:20
...
`api.md`, `database.md`, `integrations.md`, `operations.md`는 각 영역의 전체 대상과 판정(`KEEP`/`REDESIGN`/`DROP`/`DEFER`), 그리고 이관 진척(`완료`/`진행중`/`미착수`/`해당없음`)을 관리한다. **판정과 진척은 다른 열이다** — 판정은 "무엇을 할 것인가", 진척은 "어디까지 했는가"이고, 한 열에 섞으면 둘 다 못 읽는다. 표의 세부 열은 영역별로 다르며, 최소한 대상 식별, 사용·소유 도메인 또는 현재 구성, 위험·후속 확인을 기록한다. 근거는 API처럼 항목별로 필요한 영역에 기록하고, 데이터처럼 공통 출처인 경우 문서의 추출 범위에 기록한다. 인벤토리 내용을 `work/`나 `domains/`에 복제하지 않는다.

인벤토리의 기준 시점은 문서의 시간정보 헤더로 관리한다. 특정 항목을 작업에서 조사·구현·재판정했다면 해당 행에 관련 `work/` 링크와 재검토 조건을 추가한다.

`이관 진척` 열의 `완료`는 실제 코드와 검증 결과가 있을 때만 적는다. **추정으로 채우면 진척표가 오히려 사실을 가린다** — 확인하지 못했으면 `미착수`나 `진행중`으로 두고 무엇이 남았는지 후속 확인에 적는다. 진척 외에 확인 상태나 날짜 열을 새로 추가하지는 않는다.
