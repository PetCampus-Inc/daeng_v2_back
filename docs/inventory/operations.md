...
| S3(운영 제공) | 레거시: IAM 사용자 장기 access key를 배포 시 EC2 호스트 env(`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`)로 주입, `DefaultAWSCredentialsProviderChain`이 픽업. ECR 로그인과 동일 키. 신규 서버: 코드([`KD3-478`](../work/KD3-478-s3-infra-image-upload.md))는 `DefaultCredentialsProvider` + `aws.s3.{region,bucket}` env로 동작하나 배포 파이프라인이 없어 운영 자격증명 배선 미완료 | 필요 | `REDESIGN` | `미착수` | 신규 서버 배포 파이프라인(위 "애플리케이션 배포" 행)에 종속. 자격증명 값은 문서에 적지 않음 | 신규 배포 시 필요 env: `AWS_S3_REGION`, `AWS_S3_BUCKET`, 자격증명(env 또는 태스크/인스턴스 역할). IAM 정책은 사용 버킷에 `s3:PutObject`/`GetObject`/`HeadObject`/`CopyObject`/`DeleteObject` 필요. **전용 버킷 신설 시**: 새 키는 불필요(IAM 주체에 귀속), 새 버킷 생성 + IAM 정책에 버킷 ARN 추가 + `AWS_S3_BUCKET` 교체 + 기존 콘텐츠 `aws s3 sync`(key layout 유지 시 DB `s3_key` 무수정). 수동 생성/IaC 관리 필요 여부 함께 결정 |
