> 생성: 2026-08-02 13:45 · 최종 수정: 2026-09-09 10:00
...
| 애플리케이션 배포 | 신규 서버: Dockerfile, 이미지 빌드, 배포 파이프라인 없음. 레거시: `dev` push → GitHub Actions가 gradle build → ECR `:latest` push → EC2에 SSH로 `docker-compose up -d` | 필요 | `REDESIGN` | `미착수` | 레거시는 `:latest` 단일 태그라 이전 이미지로 되돌릴 수 없고, `down` 후 `up`이라 배포 중 downtime이 있음 | 신규는 커밋 SHA 태그와 롤백 가능한 배포 방식을 전제로 설계. 무중단 컷오버는 [`0008`](../adr/0008-무중단-컷오버-전략.md)과 함께 확정. **컨테이너에 `TZ=Asia/Seoul` 주입은 필수** — 응답의 `LocalDateTime`/`LocalTime`은 시간대 정보가 없어 KST 벽시계로 해석되며([`api-contract.md`](../conventions/api-contract.md) §2), 이게 없으면 `now()`류가 UTC로 찍혀 9시간 어긋난다. 앱 코드는 시간대를 강제하지 않는다([`KD3-495`](../work/KD3-495-response-datetime-format.md)) |
...
| S3(운영 제공) | 레거시: IAM 사용자 장기 access key를 배포 시 EC2 호스트 env(`AWS_ACCESS_KEY_ID`/`AWS_SECRET_ACCESS_KEY`)로 주입, `DefaultAWSCredentialsProviderChain`이 픽업. ECR 로그인과 동일 키. 신규 서버: 코드([`KD3-478`](../work/KD3-478-s3-infra-image-upload.md))는 `DefaultCredentialsProvider` + `aws.s3.{region,bucket}` env로 동작하나 배포 파이프라인이 없어 운영 자격증명 배선 미완료 | 필요 | `REDESIGN` | `미착수` | 신규 서버 배포 파이프라인(위 "애플리케이션 배포" 행)에 종속. 자격증명 값은 문서에 적지 않음 | 신규 배포 시 필요 env: `AWS_S3_REGION`, `AWS_S3_BUCKET`, 자격증명(env 또는 태스크/인스턴스 역할). IAM 정책(`HeadObject`·`CopyObject`는 IAM action이 아님): 객체 리소스(`arn:aws:s3:::<bucket>/*`)에 `s3:GetObject`(다운로드·존재 확인 HeadObject·commit copy source), `s3:PutObject`(업로드·commit copy destination), `s3:DeleteObject`(commit 시 원본 삭제) — 그리고 버킷 리소스(`arn:aws:s3:::<bucket>`)에 `s3:ListBucket`(없는 key HeadObject가 403이 아니라 404를 내도록). **전용 버킷 신설 시**: 새 키는 불필요(IAM 주체에 귀속), 새 버킷 생성 + IAM 정책에 버킷 ARN 추가 + `AWS_S3_BUCKET` 교체 + 기존 콘텐츠 `aws s3 sync`(key layout 유지 시 DB `s3_key` 무수정). 수동 생성/IaC 관리 필요 여부 함께 결정 |
