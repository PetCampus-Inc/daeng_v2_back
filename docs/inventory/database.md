> 생성: 2026-08-02 13:45 · 최종 수정: 2026-09-10 12:00
...
| MySQL | `free_memo_photo` | 자유메모 사진 | memo/media | memo | `REDESIGN` | `완료` | → 신규 `memo_photos`(V11) | [`KD3-465`](../work/KD3-465-memo.md) | 메모 텍스트와 독립 기능 — `(user_code, target_id)` 직접 키(memo_id 없음), 개별 추가/삭제, 유치원당 ≤5. `object_key`=media commit 후 영구 key. 삭제 시 S3 object도 제거. 저장 실패로 참조 끊긴 orphan 정리 주체는 미결 |
...
| MySQL | `checklist_submission` | 체크리스트 제출 내역 | memo | memo | `REDESIGN` | `완료` | → 신규 `checklist_submissions`(V12) | [`KD3-465`](../work/KD3-465-memo.md) | `(user_code, target_id)` 1행 원자적 upsert(전체 교체). `template_version` 보존 |
| MySQL | `checklist_answer` | 체크리스트 답변 | memo | memo | `REDESIGN` | `완료` | → `checklist_submissions.answers` `TEXT` 컬럼에 통합 | [`KD3-465`](../work/KD3-465-memo.md) | 폴리모픽 4컬럼 → `{questionCode: value}` JSON 문자열. `value` 항상 문자열. 통째로만 읽고 써서 JSON 타입 대신 TEXT |
